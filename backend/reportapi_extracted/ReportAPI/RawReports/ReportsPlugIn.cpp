...
#define SIG_THOUSAND    (' ')
//--- trade commands
static const char  *ExtOperations[9]={ "buy","sell","buy limit","sell limit","buy stop","sell stop","balance","credit","error" };
//---
...
LPSTR ToSym(char *pricebuf,const int len,const double price,int digits)
  {
   char tmp[8]="";
//--- checks
   if(pricebuf!=NULL && len>1)
     {
      if(digits<0 || digits>8 || price<-10000000000 || price>10000000000) pricebuf[0]=0;
      else
        {
         //--- format string
         tmp[0]='%'; tmp[1]='.'; tmp[2]='0'+digits; tmp[3]='l'; tmp[4]='f'; tmp[5]=0;
         StringCchPrintfA(pricebuf,len-1,tmp,price);  // formatting
        }
     }
//--- return buffer
   return(pricebuf);
...
//+------------------------------------------------------------------+
//| ѕредставление объЄма в строку                                    |
//+------------------------------------------------------------------+
LPSTR ToVolume(const double value,char* buffer,const int buffer_len)
  {
   char *cp;
//--- проверки
   if(buffer==NULL || buffer_len<1) return(buffer);
//--- провер€ем если меньше 1000 то просто пишем
   if(value<1000)
     {
      StringCchPrintfA(buffer,buffer_len,"%.2lf",value);
     }
   else
      if(value<1000000)
        {
         //--- распишем по тыс€чам
         StringCchPrintfA(buffer,buffer_len,"%.5lf",value/1000.0);
         int len=strlen(buffer);
         for(cp=buffer+len-1;cp>=buffer;cp--)
            if(*cp!='0' || *cp=='.')
              {
               if(*cp!='.') cp++;
               break;
        ...
        {
         //--- распишем по миллионам
         StringCchPrintfA(buffer,buffer_len,"%.8lf",value/1000000.0);
         int len=strlen(buffer);
         for(cp=buffer+len-1;cp>=buffer;cp--)
            if(*cp!='0' || *cp=='.')
              {
...
//|                                                                  |
//+------------------------------------------------------------------+
LPCSTR FormatOrderReason(char reason,char *sz,int maxchars)
  {
   if(sz==NULL || maxchars<=0) return(sz);
//---
   switch(reason)
     {
      case TR_REASON_CLIENT:
         strncpy_s(sz,maxchars,"Client",_TRUNCATE);
         break;
      case TR_REASON_EXPERT:
         strncpy_s(sz,maxchars,"Expert",_TRUNCATE);
         break;
      case TR_REASON_DEALER:
         strncpy_s(sz,maxchars,"Dealer",_TRUNCATE);
         break;
      case TR_REASON_SIGNAL:
         strncpy_s(sz,maxchars,"Signal",_TRUNCATE);
         break;
      case TR_REASON_GATEWAY:
         strncpy_s(sz,maxchars,"Gateway",_TRUNCATE);
         break;
      default:
         *sz='\0';
         break;
