...
CREATE TABLE IF NOT EXISTS transaction_wallet (
  ...
  sender_acct      TEXT,      -- ocr_sender_acct (shared card -> network/family)
  method           TEXT,
...
cur.execute("""
INSERT INTO transaction_wallet (transaction_id, wallet_id, sender_acct, method, receipt_filename, source, confidence)
...
cand AS (
  SELECT t.id AS txn_id, dr.wallet_id, dr.sender_acct, dr.method, dr.receipt_filename, dr.source,
...
SELECT txn_id, wallet_id, sender_acct, method, receipt_filename, source,
