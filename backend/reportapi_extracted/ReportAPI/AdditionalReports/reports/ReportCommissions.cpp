...
//+------------------------------------------------------------------+
#include "stdafx.h"
#include "ReportCommissions.h"
//+------------------------------------------------------------------+
//| Commissions Report                                               |
//+------------------------------------------------------------------+
BOOL CReportCommissions::GenerateHTML(const ReportParams* params)
  {
   char    tmp[256]="";
   int     i,j,isg,total_count=0,result=TRUE;
   ...
   for(i=0;i<params->group.total;i++)
     {
      ...
      if(ur!=NULL && (grp=GroupGet(params,ur->group))!=NULL)
        {
         for(isg=0; isg<MAX_SEC_GROUPS; isg++)
           {
            fprintf(m_file,"%.2lf x %.2lf",slots[isg]/100.0,grp->secgroups[isg].comm_base);
            if((isg+1)%8==0) fprintf(m_file,"<br>");
            else             fprintf(m_file,", ");
           }
        }
      fprintf(m_file,"</td>");
...
//|                                                                  |
//+------------------------------------------------------------------+
BOOL CReportCommissions::GenerateCSV(const ReportParams* params)
  {
   char    tmp[256]="";
   int     i,j,isg,total_count=0,result=TRUE;
   double  total_comm=.0,total_agent=.0,comm=.0,agent=.0;
...
//--- checks
   if(Startup(params)==FALSE) return(FALSE);
//--- report header
   fprintf(m_file,"Commissions Report for '%s'",params->group.name);
   FormatDateTime(params->group.from,tmp,sizeof(tmp)-1,FALSE);
   fprintf(m_file," from %s",tmp);
   FormatDateTime(params->group.to-1,tmp,sizeof(tmp)-1,FALSE);
   fprintf(m_file," to %s\n",tmp);
   ...
   for(i=0;i<params->group.total;i++)
     {
      ...
      ur=UserRecordGet(params, params->group_logins[i]);
      ...
      if(ur!=NULL && (grp=GroupGet(params,ur->group))!=NULL)
        {
         for(isg=0; isg<MAX_SEC_GROUPS; isg++)
           {
            fprintf(m_file,"%.2lf x %.2lf",slots[isg]/100.0,grp->secgroups[isg].comm_base);
            if((isg+1)<MAX_SEC_GROUPS) fprintf(m_file," / ");
           }
        }
      fprintf(m_file,";");
      //--- total & agent
