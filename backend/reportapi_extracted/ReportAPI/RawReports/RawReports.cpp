...
//+------------------------------------------------------------------+
#include "stdafx.h"
#include "reports\ReportOnline.h"
#include "reports\ReportUsers.h"
#include "reports\ReportOrders.h"
#include "reports\ReportRaw.h"
#include "reports\ReportHistory.h"
#include "reports\ReportJournal.h"
//+------------------------------------------------------------------+
//| DLL exports                                                      |
//+------------------------------------------------------------------+
extern "C"
   ...
   switch(dwReason)
     {
      case DLL_PROCESS_ATTACH:
         if(ExtReports==NULL)
           {
            ExtReports=new CReportOnline(NULL,"Online Users");
            ExtReports=new CReportUsers(ExtReports,"Accounts Report");
            ExtReports=new CReportOrders(ExtReports,"Orders Report");
            ExtReports=new CReportRaw(ExtReports,"Raw Report");
            ExtReports=new CReportHistory(ExtReports,"History Report");
            ExtReports=new CReportJournal(ExtReports,"Journal Report");
           }
         break;
...
//---
   if(buffer!=NULL && ccmax>1)
     {
      strncpy(buffer,"Raw Reports",ccmax);
      buffer[ccmax-1]=0;
     }
//---
