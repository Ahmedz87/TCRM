...
#define ManAPIProgramBuild    1473
...
struct ConDataServer
  {
   ...
   char              iswitness;              // failover witness flag
...
enum
  {
   ...
   RET_INVALID_COMPANY=15,          // company name does not belong to a license or whitelabel
   //--- account status
...
struct UserRecord
  {
   ...
   int               enable_flags;               // additional access flags
   int               enable_reserved[1];         // for future use
   //---
...
//+------------------------------------------------------------------+
//| UserRecord additional access flags                               |
//+------------------------------------------------------------------+
enum EnUserFlags
  {
   ...
   inline void Init(LPCSTR lib_path=NULL)
     {
      ...
      if(lib_path!=NULL) 
        {
         strncpy_s(path,_countof(path),lib_path,_TRUNCATE);
         path[sizeof(path)-1]=0;
        }
      else               
        { 
         #ifndef _WIN64
         strncpy_s(path,_countof(path),"mtmanapi.dll",_TRUNCATE); 
         path[sizeof(path)-1]=0; 
         #else
         strncpy_s(path,_countof(path),"mtmanapi64.dll",_TRUNCATE); 
         path[sizeof(path)-1]=0; 
         #endif
        }
      //---
