    ...
    <div style={{ color:'#e8e8e8' }}>
      ...
      {stats && (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(6,1fr)', gap:10, marginBottom:16 }}>
          {[
            ...
            <div key={label} style={{ background:'var(--bg-card,#111318)', border:'1px solid var(--border,#222)', borderRadius:10, padding:'12px 14px' }}>
        ...
        {TABS.map(([k,label])=>(
          <button key={k} onClick={()=>setSev(k)}
            style={{ padding:'6px 14px', borderRadius:8, fontSize:12, cursor:'pointer',
              ...
              border:`1px solid ${sev===k?'#00aaff':'#333'}`,
              color: sev===k?'#00aaff':'#aaa' }}>{label}</button>
        ))}
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search login..."
          style={{ marginLeft:'auto', padding:'6px 12px', borderRadius:8, background:'var(--bg-input,#1a1d24)', border:'1px solid #333', color:'#fff', fontSize:12 }} />
      </div>

      {/* Traders table */}
      <div style={{ background:'var(--bg-card,#111318)', border:'1px solid var(--border,#222)', borderRadius:10, overflow:'hidden' }}>
        <div style={{ display:'grid', gridTemplateColumns:'70px 1fr 90px 90px 90px 100px 1fr', gap:8, padding:'10px 14px', fontSize:10, color:'#666', textTransform:'uppercase', borderBottom:'1px solid #222' }}>
        ...
        <div style={{ maxHeight:560, overflowY:'auto' }}>
          {traders.map(t=>(
            <div key={t.login} onClick={()=>openDetail(t.login)}
              style={{ display:'grid', gridTemplateColumns:'70px 1fr 90px 90px 90px 100px 1fr', gap:8, padding:'11px 14px', fontSize:12.5, borderBottom:'1px solid #181b21', cursor:'pointer', alignItems:'center' }}
      ...
      {detail && (
        <div onClick={()=>setDetail(null)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', zIndex:100, display:'flex', justifyContent:'flex-end' }}>
          <div onClick={e=>e.stopPropagation()} style={{ width:680, maxWidth:'94vw', height:'100%', overflowY:'auto', background:'#0d0f14', borderLeft:'1px solid #222', padding:24 }}>
            {detail.loading ? <div style={{ color:'#888' }}>Loading…</div> : detail.error ? <div style={{ color:'#ff5d6c' }}>Not found</div> : (<>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:10, marginBottom:16 }}>
                {[['Score',detail.score,'/100'],['Hedged',Math.round(detail.hedged_ratio*100)+'%',`${detail.hedged_positions}/${detail.total_positions}`],['Bonus',detail.has_bonus?fmt(detail.credit):'none','']].map(([l,v,s]:any)=>(
                  <div key={l} style={{ background:'#15181f', borderRadius:8, padding:'12px 14px' }}>
              ...
              {detail.hedge_trades?.length>0 && (
                <div style={{ marginBottom:16 }}>
                  ...
                  <div style={{ background:'#15181f', borderRadius:8, overflow:'hidden' }}>
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 80px', gap:6, padding:'7px 10px', fontSize:9, color:'#666', textTransform:'uppercase', borderBottom:'1px solid #222' }}>
                    ...
                    <div style={{ maxHeight:300, overflowY:'auto' }}>
                      {detail.hedge_trades.map((h:any,i:number)=>(
                        <div key={i} style={{ display:'grid', gridTemplateColumns:'1fr 1fr 80px', gap:6, padding:'8px 10px', fontSize:11, borderBottom:'1px solid #1a1d24', alignItems:'center' }}>
                          <span>
                          ...
                          <span>
                            ...
                            {h.relation!=='same_symbol' && <span style={{ fontSize:8, color:'#888', padding:'1px 4px', background:'#222', borderRadius:3, marginLeft:4 }}>{h.relation}</span>}
                          </span>
              ...
              {detail.connections?.length>0 && (
                <div style={{ marginBottom:16 }}>
                  ...
                  {detail.connections.map((c:any,i:number)=>{
                    ...
                    return (
                      <div key={i} style={{ display:'flex', alignItems:'center', gap:8, padding:'6px 10px', background:'#15181f', borderRadius:7, marginBottom:4, fontSize:11 }}>
